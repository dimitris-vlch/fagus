...
# Ορίζοντας τώρα το file, ως json.load(file), το μετατρέπουμε σε python αντικείμενο, μιά λίστα την οποία και στην συνέχεια την ορίζουμε ως data.
with open(json_file, "r", encoding="utf-8") as file:
    data = json.load(file)

# Βήμα 4: Έλεγχος αν το αρχείο json περιέχει δεδομένα.
# Το βήμα αυτό δεν είναι απαραίτητο, το χρησιμοποιούμε αν θέλουμε να αποτρέψουμε την δημιουργία κενών αρχείων csv,
# από λήψεις κενών json αρχείων. Σε περίπτωση που το Json είναι άδειο (if not data:) τότε θα σταματήσει το script sys.exit(1), αφού θα εκτυπώσει το μήνυμα με print.
if not data: 
    print("Το JSON αρχείο είναι κενό. Διακοπή μετατροπής.")
    sys.exit(1)

# Βήμα 5: Άνοιγμα και write για το αρχείο csv.
# Θα δημιουργήσει αρχείο results_sample_json.csv, σε λειτουργία εγγραφής “w”, που δέχεται ειδικούς χαρακτήρες encoding="utf-8". 
# To newline="" αποτρέπει την εισαγωγή κενών γραμμών.
with open(csv_file, "w", encoding="utf-8", newline="") as file:
    ...
    # writer. Ο πρώτος όρος fieldnames αναφέρεται στα δεδομένα ("sample_accession", "country", "location") και το fieldnames=fieldnames εξηγεί στο writer ότι πρέπει
    # να τα γράψει στο csv όπως έχουν.
    writer = csv.DictWriter(file, fieldnames=fieldnames)
    
    # 5.3: με το writeheader εξηγούμε ότι τα παραπάνω είναι η πρώτη γραμμή του csv.
    writer.writeheader()
    
    # 5.4: Συμπλήρωση από τη δεύτερη μέχρι και την τελευταία στήλη του csv. Το row αναφέρεται σε κάθε dictionary της λίστας ένα-ένα με τη σειρά, συγκεκριμένα
    # της λίστας data. Η writerow δεν πειράζει την πρώτη γραμμή γιατί πριν χρησιμοποιήσαμε τη writeheader και αξιοποιεί την write όπως την ορίσαμε παραπάνω. Γράφει κάθε
