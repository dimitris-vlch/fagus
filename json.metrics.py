...
def count_samples_with_location(json_file):
    ...
    try: #sosaki το try και το except πρέπει να έχουν την ίδια εσοχή! 
        with open(json_file, "r", encoding="utf-8") as file:
            ...
            count_with_location = sum (1 for sample in data sampleget("location")) 
        ...
        print(f"Σφάλμα κατά την ανάγνωση του αρχείου: {e}")
# Βήμα 14: Λήψη του ονόματος αρχείου από την γραμμή εντολών. Το όνομα που δίνει ο χρήστης ορίζεται ως η μεταβλητή json_file, η οποία και είναι το όρισμα της συνάρτησης που μετράει τον αριθμό των δειγμάτων. Έτσι, ο χρήστης επιλέγει ποιο θα είναι το αρχείο json για το οποίο τα δείγματα θα μετρηθούν.
json_file = sys.argv[1]
# Βήμα 15: Καλούμε τις συνάρτησεις, προκειμένου να μετρήσουμε τον αριθμό των δειγμάτων, καθώς και τον αριθμό των δειγμάτων που παρουσιάζουν τοποθεσία, για τη μεταβλητή json_file.
