...
colors = ["#f0e442", "#a65628"]  # έντονο κίτρινο & σοκολατί καφέ
...
plt.title("Percentage of registries with or without\nboth country and coordinate information")
...
plt.figtext(0.5, 0.03, f"Total registries: {total_registries}", ha='center', fontsize=12)
...
colors = ["#ffd92f", "#e31a1c"]  # έντονο κίτρινο & βαθύ κόκκινο


fig = plt.figure(figsize=(6, 6)) # ορίζω fig = ώστε μετά να έχω προσβασε σε fig.add_axes() για την τροποποίηση της θέσης του λόγκο.
plt.pie(sizes, labels=labels, autopct= lambda pct: f"{pct:.1f}% ({int(pct / 100. * sum(sizes))})", startangle=00, colors = colors)
plt.title("Agreement vs disagreement between\ncoordinates and declared country")
plt.figtext(0.5, 0.02, f"Total registries: {total_registries}", ha='center', fontsize=12)
plt.figtext(0.5, 0.07, f"Number of registries with both coordinates and country: {len(curated_data)}", ha='center', fontsize=12)
...
plt.tight_layout()
plt.savefig("true_and_faultycoordinates_pie_chart.png", dpi=300)
plt.show()
